...
if platform.mac_ver()[0] > '10.':
    sys.exit(os.system(
        'osascript -e \'display dialog "MacOS not supported" buttons {"OK"} default button 1 with icon caution with title "Trojan GTK"\''))
...
try:
    ...
    sys.exit(os.system(
        'gdialog --title "Trojan GTK" --msgbox "Please install python-gtk2" 15 60'))
try:
    import pynotify
    pynotify.init('Trojan Notify')
except ImportError:
    pynotify = None
try:
    import appindicator
...
def rewrite_desktop(filename):
    ...
    with open(filename, 'wb') as fp:
        content = re.sub(r'(?m)Exec=.*', '''Exec=bash -c "export PATH=$PATH:`dirname '%%k'`:%s; exec trojan-gtk.py"''' % os.getcwd(), content)
        content = re.sub(r'Icon=\S*', 'Icon=%s/trojan-gtk.png' % os.getcwd(), content)
        fp.write(content)
...
class TrojanGTK:
    ...
    command = [
        os.path.join(os.path.dirname(os.path.abspath(__file__)), 'trojan'), 'config.json']
    message = u'Trojan already started.'
    fail_message = u'Trojan start failed, see the terminal'
    ...
    def __init__(self, window, terminal):
        ...
        logo_filename = os.path.join(
            os.path.abspath(os.path.dirname(__file__)), 'trojan-gtk.png')
        self.window.set_icon_from_file(logo_filename)

        if appindicator:
            self.trayicon = appindicator.Indicator(
                'Trojan', 'indicator-messages', appindicator.CATEGORY_APPLICATION_STATUS)
            ...
            self.trayicon.set_tooltip('Trojan')
    ...
    def show_notify(self, message=None, timeout=None):
        if pynotify and message:
            notification = pynotify.Notification('Trojan Notify', message)
    ...
    def on_reload(self, widget, data=None):
        if self.childexited:
            self.terminal.disconnect(self.childexited)
        os.system('kill -1 %s' % self.childpid)
...
def main():
    ...
    if os.path.isfile('trojan-gtk.desktop'):
        rewrite_desktop('trojan-gtk.desktop')

    TrojanGTK(gtk.Window(), vte.Terminal())
    gtk.main()
